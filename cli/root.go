// Package cli builds the jay command tree on top of the jayalammar library.
package cli
...
type App struct {
	client *jayalammar.Client
	cfg    jayalammar.Config
...
func Root() *cobra.Command {
	app := &App{cfg: jayalammar.DefaultConfig()}

	root := &cobra.Command{
		Use:   "jay",
		Short: "Browse Jay Alammar machine learning blog",
		Long: `jay reads Jay Alammar's machine learning blog through its public Atom feed.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

jay is an independent tool and is not affiliated with Jay Alammar.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = jayalammar.NewClient(a.cfg)
	return nil
}
