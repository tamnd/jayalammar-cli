...
describes what users get. Pick a format with `-o`, or let jay choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jay <command> -o table   # aligned columns for reading
jay <command> -o jsonl   # one JSON object per line, for piping
jay <command> -o json    # a single JSON array
jay <command> -o csv     # spreadsheet friendly
jay <command> -o tsv     # tab-separated
jay <command> -o url     # just the URL column
jay <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jay <command> --fields id,title,url
```
...
```bash
jay <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jay <command>            # a table, because this is a terminal
jay <command> | wc -l    # JSONL, because this is a pipe
```
